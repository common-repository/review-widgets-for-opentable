...
class TrustindexWidget_opentable extends WP_Widget {
...
{
parent::__construct(
'trustindex_opentable_widget',
'Trustindex - Opentable reviews widget',
[
'classname' => 'trustindex-widget',
'description' => 'Embed Opentable reviews fast and easily into your WordPress site. Increase SEO, trust and sales using Opentable reviews.'
]
);
}
...
{
global $wpdb;
global $trustindex_pm_opentable;
if ($trustindex_pm_opentable->is_enabled()) {
extract($args);
echo $before_widget;
$wasError = false;
...
echo $trustindex_pm_opentable->renderWidgetFrontend($instance['ti-widget-ID']);
} else if ($trustindex_pm_opentable->is_noreg_linked()) {
echo $trustindex_pm_opentable->renderWidgetFrontend();
} else {
echo $trustindex_pm_opentable->frontEndErrorForAdmins(sprintf(__("Please fill out <strong>all the required fields</strong> in the <a href='%s'>widget settings</a> page", 'trustindex-plugin'), admin_url('admin.php?page='.$trustindex_pm_opentable->get_plugin_slug().'/settings.php')));
}
echo $after_widget;
} else {
}
...
global $wp_version;
global $trustindex_pm_opentable;
$tiWidgets = $trustindex_pm_opentable->get_trustindex_widgets();
...
<?php if ($trustindex_pm_opentable->is_trustindex_connected()): ?>
<?php if ($tiWidgets): ?>
...
<?php endforeach; ?>
<?php else: ?>
<?php echo TrustindexPlugin_opentable::get_alertbox('warning',
...
<img src="<?php echo $trustindex_pm_opentable->get_plugin_file_url('static/img/help-where-is-id.jpg'); ?>" alt="ID column here: <?php echo 'https://admin.trustindex.io/'; ?>widget" />
</div>
<?php else: ?>
<?php echo TrustindexPlugin_opentable::get_alertbox('warning',
...
. sprintf(__("Go to <a href='%s'>plugin setup page</a> to complete the one-step setup guide and enjoy the full functionalization!", 'trustindex-plugin'), admin_url('admin.php?page='.$trustindex_pm_opentable->get_plugin_slug().'/settings.php&tab=setup_trustindex'))
); ?>
<?php endif; ?>
</div>
<?php
